...
cp -R ./chiko ${KERNELDIR}/output/$TARGET/
cp ./$TARGET/boot.img ${KERNELDIR}/output/$TARGET/chiko
...
tar -H ustar -c ${KERNELDIR}/output/$TARGET/chiko/boot.img > SM-$TARGET-$AUDIO-kernel-${GETVER}-`date +[%d-%m-%y]`.tar
...
if [ "$mc" = "Y" -o "$mc" = "y" ]; then
	gnome-terminal -e make clean
	gnome-terminal -e make mrproper
fi
